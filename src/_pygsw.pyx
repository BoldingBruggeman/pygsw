...
cdef extern void cgws_sa_from_sp(int n, double* sp, double* p, double* long, double* lat, double* sa)
cdef extern void cgws_pt0_from_t(int n, double* sa, double* t, double* p, double* pt)
cdef extern void cgws_ct_from_pt(int n, double* sa, double* pt, double* ct)
cdef extern void cgws_nsquared_3d(int nx, int ny, int nz, double* h, double* sa, double* ct, double* p, double* lat, double* n2)
...
def sa_from_sp(const double[::1] lon not None, const double[::1] lat not None, const double[::1] p not None, const double[::1] sp not None, double[::1] out=None):
    ...
    cgws_sa_from_sp(out.shape[0], &sp[0], &p[0], &lon[0], &lat[0], &out[0])
    return out
...
def nsquared(const double[:,:,::1] h not None, const double[:,:,::1] SA not None, const double[:,:,::1] ct not None, const double[:,:,::1] p not None, const double[:,::1] lat not None, double[:,:,::1] out=None):
    ...
    cgws_nsquared_3d(h.shape[2], h.shape[1], h.shape[0], &h[0,0,0], &SA[0,0,0], &ct[0,0,0], &p[0,0,0], &lat[0,0], &out[0,0,0])